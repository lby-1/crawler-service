...
/**
 * 云南省公共资源交易中心 - 招标计划处理器
 * 列表API: getTenserPlanList
 * 详情API: getTenserPlanDetail?guid=xxx
 */
@Slf4j
@Component
public class YnggzyZbjhHandler implements CrawlHandler {

    private static final String[] SITE_CODES = {"ynggzy", "ynggzy-slgc"};
    private static final String CATEGORY_CODE = "getTenserPlanList";
    private static final String DETAIL_API = "https://ggzy.yn.gov.cn/ynggfwpt-home-api/jyzyCenter/jyInfo/gcjs/getTenserPlanDetail?guid=";
    ...
    @Override
    public String getHandlerName() { return "云南交易中心-招标计划处理器"; }
    ...
    private void fetchDetail(CrawlItem item, int reqInterval) {
        ...
        try {
            ...
            if (item.getProjectName() == null && item.getTitle() != null) {
                item.setProjectName(item.getTitle().replaceAll("招标(项目)?计划$", "").trim());
            }

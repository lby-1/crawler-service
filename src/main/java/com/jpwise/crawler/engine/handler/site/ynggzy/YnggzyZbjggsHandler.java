...
/**
 * 云南省公共资源交易中心 - 中标结果公告处理器
 * 列表API: getZbJgGgList
 * 详情API: findZbJgGgByGuid?guid=xxx
 * 特点：JSON有 winbiddername/tenderprojectname 等直接字段 + bulletincontent(HTML)
 */
@Slf4j
@Component
public class YnggzyZbjggsHandler implements CrawlHandler {

    private static final String[] SITE_CODES = {"ynggzy", "ynggzy-slgc"};
    private static final String CATEGORY_CODE = "getZbJgGgList";
    private static final String DETAIL_API = "https://ggzy.yn.gov.cn/ynggfwpt-home-api/jyzyCenter/jyInfo/gcjs/findZbJgGgByGuid?guid=";
    ...
    @Override
    public String getHandlerName() { return "云南交易中心-中标结果公告处理器"; }
    ...
    private void fetchDetail(CrawlItem item, int reqInterval) {
        ...
        try {
            ...
            if (item.getProjectName() == null && item.getTitle() != null) {
                item.setProjectName(item.getTitle().replaceAll("中标结果公[告示]$", "").trim());
            }

...
/**
 * 广西公共资源交易中心 - 中标结果公示处理器
 * 列表：API引擎
 * 详情：HTML页面（div.ewb-details-info）
 * 正文格式：中文编号段落，包含中标人、中标价等最终结果信息
 */
@Slf4j
@Component
public class GxggzyZbjggsHandler implements CrawlHandler {

    private static final String[] SITE_CODES = {"gxggzy", "gxggzy-slgc"};
    private static final String CATEGORY_CODE = "43236";
    ...
    private void fetchHtmlDetail(CrawlItem item, int reqInterval) {
        ...
        try {
            ...
            // 中标结果公示特有：提取中标人和中标价
            GxggzyExtractUtils.extractBidResultFromText(item, textContent);

            // 项目名称 fallback
            if (item.getProjectName() == null && item.getTitle() != null) {
                String title = item.getTitle();
                for (String suffix : List.of("中标结果公示", "中标结果公告", "中标公示", "中标公告")) {

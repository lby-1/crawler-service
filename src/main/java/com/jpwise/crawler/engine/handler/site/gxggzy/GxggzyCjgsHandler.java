...
/**
 * 广西公共资源交易中心 - 成交公示处理器（中标候选人公示）
 * 列表：API引擎
 * 详情：HTML页面（div.ewb-details-info）
 * 正文格式：中文编号段落，包含"评标结果"、"第一中标候选人"等信息
 */
@Slf4j
@Component
public class GxggzyCjgsHandler implements CrawlHandler {

    private static final String[] SITE_CODES = {"gxggzy", "gxggzy-slgc"};
    private static final String CATEGORY_CODE = "43235";
    ...
    public boolean supports(String siteCode, String categoryCode) {
        for (String code : SITE_CODES) {
            if (code.equals(siteCode)) {
                return categoryCode != null && categoryCode.contains(CATEGORY_CODE);
            }
        }
        return false;
    }
    ...
    public void crawlDetail(CrawlItem item, CrawlerProperties.SiteConfig site, int reqInterval) {
        ...
        log.info("[{}] 详情获取完成: 项目={}, 中标人={}, 中标价={}, 招标人={}, 联系人={}, 电话={}",
    ...
    private void fetchHtmlDetail(CrawlItem item, int reqInterval) {
        ...
        try {
            ...
            // 成交公示特有：提取第一中标候选人
            GxggzyExtractUtils.extractBidCandidateFromText(item, textContent);

            // 项目名称 fallback
            if (item.getProjectName() == null && item.getTitle() != null) {
                String title = item.getTitle();
                for (String suffix : List.of("中标候选人公示", "成交公示", "中标公示")) {

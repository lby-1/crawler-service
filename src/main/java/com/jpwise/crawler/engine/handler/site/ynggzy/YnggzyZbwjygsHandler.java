...
/**
 * 云南省公共资源交易中心 - 招标文件预公示处理器
 * 列表API: getZbwjygsList
 * 详情API: getZbwjygsDetail?guid=xxx
 */
@Slf4j
@Component
public class YnggzyZbwjygsHandler implements CrawlHandler {

    private static final String[] SITE_CODES = {"ynggzy", "ynggzy-slgc"};
    private static final String CATEGORY_CODE = "getZbwjygsList";
    private static final String DETAIL_API = "https://ggzy.yn.gov.cn/ynggfwpt-home-api/jyzyCenter/jyInfo/gcjs/getZbwjygsDetail?guid=";
    ...
    @Override
    public String getHandlerName() { return "云南交易中心-招标文件预公示处理器"; }
    ...
    public void crawlDetail(CrawlItem item, CrawlerProperties.SiteConfig site, int reqInterval) {
        ...
        log.info("[{}] 详情完成: 项目={}, 城市={}, 招标人={}",
            getHandlerName(), item.getProjectName(), item.getCity(), item.getTenderer());
    }
    ...
    private void fetchDetail(CrawlItem item, int reqInterval) {
        ...
        try {
            ...
            if (item.getProjectName() == null && item.getTitle() != null) {
                item.setProjectName(item.getTitle().replaceAll("招标文件预公示$", "").trim());
            }
